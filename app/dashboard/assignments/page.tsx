...
export default function AssignmentsPage() {
  ...
  const handleAddVendor = async () => {
    ...
    try {
      ...
      toast({
        title: "Vendor Added",
        description: `${data.vendor.full_name} has been added`,
        variant: "success",
      });
  ...
  const fetchData = async () => {
    try {
      ...
      // Get organization - try multiple approaches
      ...
      // Fetch ALL profiles with role vendor (not filtering by org for testing)
      const { data: vendorData, error: vendorError } = await supabase
        .from("profiles")
        .select("id, full_name")
        .eq("role", "vendor");

      console.log("Vendors fetched:", vendorData, vendorError);
  ...
  const extractDriveInfo = (link: string) => {
    // Extract folder/file name from Google Drive link if possible
    // This is a basic extraction - could be enhanced with Google Drive API
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const insertData: any = {
        ...
        original_drive_link: formData.driveLink,
      ...
      if (formData.clientName) insertData.client_name = formData.clientName;
      if (formData.clientEmail) insertData.client_email = formData.clientEmail;
      if (formData.dueDate) insertData.due_date = formData.dueDate;
      ...
      if (error) {
        if (error.message.includes("does not exist")) {
          throw new Error("Please run the SQL script in Supabase to create the drive_assignments table first.");
        }
        throw error;
      }
